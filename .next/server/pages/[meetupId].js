...
var MeetUpDetails_module = __webpack_require__("ZNXt");
...
async function getStaticPaths() {
  const client = await external_mongodb_["MongoClient"].connect('mongodb+srv://jajjibhai007:kamlajani007@cluster0.vaxfc.mongodb.net/meetups?retryWrites=true&w=majority');
  const db = client.db();
  const meetupCollections = db.collection('meetups');
  const selectedMeetups = await meetupCollections.find({}, {
    _id: 1
  }).toArray();
  client.close();
  return {
...
async function getStaticProps(context) {
  ...
  const db = client.db();
  const meetupCollections = db.collection('meetups');
  const showMeetUp = await meetupCollections.findOne({
    _id: Object(external_mongodb_["ObjectId"])(meetupId)
  });
  client.close(); // console.log('meetUp id'+showMeetUp._id);
...
/* harmony default export */ var _meetupId_ = __webpack_exports__["default"] = (ShowDetails);
